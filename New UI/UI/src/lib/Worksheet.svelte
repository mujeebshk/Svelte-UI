...
<!-- <div class="tabbed-area">
	...
	<ul class="tabs group">
	    <li><a href="#box-one">Recent</a></li>
	    <li><a href="#box-two">Shared with me</a></li>
	    <li><a href="#box-three">My Worksheets</a></li>
	</ul>
...
.tabs li a:hover { color: rgb(26, 108,231) }
.tabs li a:hover {border-bottom: 2px solid rgb(26, 108,231); }
.group:after { visibility: hidden; display: block; font-size: 0; content: " "; clear: both; height: 0; }
.box-wrap { position: relative; 
    /* min-height: 250px; */
 }
.tabbed-area div div { background: white; padding: 20px;
     /* min-height: 250px;  */
     position: absolute; top: -1px; left: 1em; 
     width: 100%;
     }
/* .tabbed-area div div, .tabs li a { border-bottom: 1px solid #ccc; } */
...
<script lang="ts">
    ...
    import ButtonName from "./ButtonName.svelte";
...
<WorksheetModal location={location}
open={importData}
on:close={() => {
    importData = false;
  }}
/>
<!-- <ButtonName>
    <h6 slot="title">Worksheets</h6>
    <h6 slot="name" >Worksheets</h6>
</ButtonName> -->
<Header>Worksheet
    <Search />
    <ButtonName>Worksheet</ButtonName>
</Header>
<div class='paper'>
    ...
    on:click={() => {
        ...
        <div style='margin:2em'>
            {#each { length: numberOfTables } as _, i}
            <Card>
                <div
                  class="tables"
                  style="padding-left:10px; line-height: 1.4rem ;"
                >
                  <Grid container>
                    <!-- {#await info}
                      <Grid col={12}>
                        {table} <br /> <br />
                        <BarLoader color="#1fb1d9" />
                      </Grid> -->
                    <!-- {:then info} -->
                      <Grid col={12}>
                        <div class="tablesdata">
                          ...
                          overflow: hidden;
                          text-overflow: ellipsis; cursor: pointer; text-transform:capitalize"
                          on:click={() => {
                            importData = true;
                        }}
                          >
                      ...
                      <Grid col={12}>
                        <div style="display:block ruby;">
                          <div style="max-width: 26.5em;">
                            <!-- {info.result.schema.length.toLocaleString()} columns,
                            {info.result.schema[0].stats.count.toLocaleString()} records. -->
                            columns,records.
                          </div>
                          <div style="position:relative;right:-9%;float:inline-end;">
                            <!-- {info.result.date} -->
                            date
                          </div>
                        </div>
                        <div style="position:absolute; right: 2vh; top: 1vh;" title="Delete Table">
                            <!-- <IconButton icon="fa-times" on:click={deleteTable} /> -->
                          <!-- {#if modelOpen}
                            <Modal
                              title={table}
                              open={modelOpen}
        ...
        <h3>No shared Dashboards</h3>
        {/if}
        
        {#if 3 === currentTab}
<h3>No shared Worksheets</h3>
{/if}
